<template>
  <el-upload class="single-uploader" action="" :disabled="disabled" :show-file-list="false" :accept="acceptValue" :http-request="uploadImg" :before-upload="beforeUpload">
...
export default {
  ...
  created() {
    this.prewiewUrl = this.value ? this.getDownloadUrl(this.value) : ''
  },
  watch: {
    value(val) {
      this.imageUrl = val
    ...
    imageUrl(val) {
      this.prewiewUrl = this.getDownloadUrl(val)
    }
  },
  methods: {
    uploadImg(e) {
